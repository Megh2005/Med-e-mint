  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (validateForm() && selectedPlan) {
      ...
      try {
        ...
        const file = new File(
          [blob],
          `${formData.title.replace(/\s+/g, "_")}.jpg`,
          { type: "image/jpeg" }
        );
          ...
          <CardContent className="space-y-4 pt-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {[...Array(8)].map((_, i) => (
                <div
                  key={i}
                  className="flex items-center gap-4 p-3 rounded-lg shadow-neumorphic-inset"
                >
    ...
    <div className="container mx-auto flex items-center justify-center min-h-[calc(100vh-10rem)] p-4">
      <Card className="w-full max-w-4xl bg-background shadow-neumorphic-lg">
        ...
        <CardContent className="space-y-6 pt-6">
          <div className="grid grid-cols-1 uppercase md:grid-cols-2 gap-4">
            <InfoRow
              icon={<User size={20} />}
              label="Full Name"
              value={userData?.name}
            ...
            <div className="mt-6">
              ...
              {userData?.dietPlanUrl && userData.dietPlanUrl.length > 0 ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                  {userData.dietPlanUrl.map(
                    (plan: { url: string; date: string }, index: number) => (
                      <Dialog key={index}>
      ...
      <Dialog
        ...
        onOpenChange={setConnectWalletDialogOpen}
      >
        <DialogContent>
          <DialogHeader>
